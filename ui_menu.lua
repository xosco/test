...
-- FLY functionality implementation
local function createFlyFunctionality()
    local flying = false
    local flySpeed = 100
    ...
    local function updateFlySpeed(newSpeed)
        ...
        if bodyVelocity then
            -- Recalculate current velocity with new speed
    ...
    local function startFlying()
        ...
        -- Create body gyro for rotation control
        ...
        userInputConnection = UserInputService.InputChanged:Connect(function(input)
            ...
            if input.UserInputType == Enum.UserInputType.Keyboard then
                ...
                local moveDirection = Vector3.new(0, 0, 0)
                local rotationDirection = Vector3.new(0, 0, 0)
                ...
                if UserInputService:IsKeyDown(Enum.KeyCode.E) then
                    rotationDirection = rotationDirection + Vector3.new(0, 1, 0)
                end
                ...
                if bodyGyro then
                    bodyGyro.CFrame = CFrame.fromMatrix(
                        ...
                        humanoidRootPart.CFrame.XVector,
                        humanoidRootPart.CFrame.YVector + rotationDirection,
                        humanoidRootPart.CFrame.ZVector
                    )
                end
            end
        end)
...
-- Create the fly functionality
local startFlying, stopFlying, updateFlySpeed = createFlyFunctionality()

-- Add FLY toggle to the library
function Library:CreateFlyToggle(tab)
